...
from app.tools.llm_extract import extract_fallback_with_llm
...
def make_extract_node(memory: RunMemory, adapter: SiteAdapter):
    async def extract_node(state: GraphState) -> dict:
        ...
        if products:
            ...
            memory.traces.append(_trace(job, "structured", len(products)))
            return {"extracted_products": products, "extraction_method": "structured", "stage_failed": None}
        ...
        canonical = extract_canonical(html) or job.url
        ...
        if name and skus:
            ...
            memory.traces.append(_trace(job, "selector", len(products)))
        ...
        if job.attempts < 2:
            ...
            memory.traces.append(_trace(job, "selector_failed", 0))
        ...
        memory.traces.append(_trace(job, "llm", len(products)))
        return {
